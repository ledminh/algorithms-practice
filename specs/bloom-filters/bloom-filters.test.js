...
class BloomFilter {
  constructor() {
    this.arrCheck = Array(100).fill(0);
  }

  add(string) {
    this.arrCheck[h1(string)] = 1;
    this.arrCheck[h2(string)] = 1;
    this.arrCheck[h3(string)] = 1;
  }
  contains(string) {
    if (
      this.arrCheck[h1(string)] &&
      this.arrCheck[h2(string)] &&
      this.arrCheck[h3(string)]
    ) {
      return true;
    }
